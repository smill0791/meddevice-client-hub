...
export default class CaseWizard extends NavigationMixin(LightningElement) {
    ...
    async handleSubmit() {
        ...
        try {
            ...
            for (const f of this.stagedFiles) {
                ...
                await saveFile({ parentId: caseId, fileName: f.name, base64Data: f.base64 });
            }
            this.createdCaseId = caseId;
            this.dispatchEvent(new CustomEvent('casecreated', { detail: { caseId } }));
    ...
    _validateCurrentStep() {
        ...
        if (this.isDescribeStep) {
            const inputs = Array.from(this.template.querySelectorAll('.describe-input'));
